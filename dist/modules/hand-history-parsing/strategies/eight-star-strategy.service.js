...
exports.EightStarStrategyService = void 0;
const base_1 = require("./base");
const { parseStringInt } = require('../../../common/utility/stringUtils');
class EightStarStrategyService extends base_1.BaseParser {
    constructor() {
        super();
    }
    parse(sections) {
    ...
    eightPokerSectionParser(chunks) {
        let street = null;
        let players = [];
        const emptyRegex = /(\w+)/;
        ...
        for (let line of chunks) {
            ...
            if (dateTimematch) {
                this.handData.handDate = dateTimematch[1].replace(/(\d{2}) (\d{2}) (\d{4})/, '$3/$2/$1');
            ...
            if (smallBlindMatch) {
                const playerSeat = this.handData.players.find((item) => item.playerName == smallBlindMatch[1]);
                players.map((item) => {
                ...
                this.handData.smallBlind = parseInt(smallBlindMatch[2].replace(/,/g, ''));
            }
            if (bigBlindMatch) {
                const playerSeat = this.handData.players.find((item) => item.playerName == bigBlindMatch[1]);
                players.map((item) => {
            ...
            const cardsRegex = /Dealt to (.+?) \[\s*([2-9TJQKA][csdhepto]),\s*([2-9TJQKA][csdhepto])\s*\]/;
            const handMatch = line.match(cardsRegex);
            if (handMatch) {
                const card1 = handMatch[2].match(/([2-9TJQKA])([csdhepto])/);
                const card2 = handMatch[3].match(/([2-9TJQKA])([csdhepto])/);
            ...
            const communityCardsRegex = /\*\* Dealing (flop|turn|river) \*\* \[ ([2-9TJQKAecsdhpto]+)(?:, ([2-9TJQKAecsdhpto]+))?(?:, ([2-9TJQKAecsdhpto]+))?(?: \])?/;
            const matchcard = line.match(communityCardsRegex);
            if (matchcard) {
                for (var i = 2; i < matchcard.length; i++) {
                    if (matchcard[i]) {
                        const card = matchcard[i].match(/([2-9TJQKA])([csdhepto])/);
                        if (card) {
            ...
            const showRegex = /(.+) shows \[ ([2-9TJQKA][cdhsepto]), ([2-9TJQKA][cdhsepto]) \]/;
            ...
            const muckRegex = /(.+) mucks \[ ([2-9TJQKA][cdhsepto]), ([2-9TJQKA][cdhsepto]) \]/;
...
exports.EightStarStrategyService = EightStarStrategyService;
//# sourceMappingURL=eight-star-strategy.service.js.map